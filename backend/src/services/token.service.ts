...
function signToken(payload: AuthTokenPayload, secret: Secret, expiresIn: string) {
  const options: SignOptions = { expiresIn: expiresIn as SignOptions["expiresIn"] };
  return jwt.sign(payload, secret, options);
}

export function createAccessToken(userId: string, role: AdminRole) {
  return signToken({ sub: userId, role, tokenType: "access" }, env.JWT_ACCESS_SECRET, env.JWT_ACCESS_EXPIRES_IN);
}

export function createRefreshJwt(userId: string, role: AdminRole) {
...
export function verifyAccessToken(token: string): AuthTokenPayload {
  try {
    ...
    if (payload.tokenType !== "access") throw new ApiError(401, "Invalid access token");
    return payload;
  } catch (error) {
...
export async function storeRefreshToken(token: string, userId: string, ip?: string, userAgent?: string) {
...
export async function findStoredRefreshToken(token: string) {
  return RefreshTokenModel.findOne({ tokenHash: hashRefreshToken(token) });
}

export async function revokeRefreshToken(token: string, ip?: string) {
  const stored = await findStoredRefreshToken(token);
  if (!stored || stored.revokedAt) return stored;
...
export async function revokeAllUserRefreshTokens(userId: string, ip?: string) {
...
export async function rotateRefreshToken(currentToken: string, role: AdminRole, ip?: string, userAgent?: string) {
