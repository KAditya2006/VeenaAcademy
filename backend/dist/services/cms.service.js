...
function buildFilters(config, query, publicOnly = false) {
...
export function createCmsService(config) {
    return {
        ...
        async listPublic(query, limitDefault = 50) {
        ...
        async getAdmin(id) {
            ...
            if (!record)
                throw new ApiError(404, `${config.module} record not found`);
            return record;
        },
        ...
        async publish(id, value, req) {
            const record = await config.model.findOne({ _id: id, isDeleted: false });
            if (!record)
                throw new ApiError(404, `${config.module} record not found`);
            record.set("isPublished", value);
            if (value && !record.get("publishedAt"))
                record.set("publishedAt", new Date());
        ...
        async reorder(items, req) {
            ...
            if (new Set(ids).size !== ids.length)
                throw new ApiError(422, "Reorder item ids must be unique");
            await config.model.bulkWrite(items.map((item) => ({ updateOne: { filter: { _id: new Types.ObjectId(item.id), isDeleted: false }, update: { $set: { sortOrder: item.sortOrder, updatedBy: req.user?._id } } } })));
