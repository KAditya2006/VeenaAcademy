...
const manageRoles = ["super_admin", "admin", "content_manager"];
const readRoles = ["super_admin", "admin", "content_manager", "counsellor", "faculty"];
export function createCmsRouter(controller, schemas, options = {}) {
    const router = Router();
    router.get("/public", validate(querySchema), controller.listPublic);
    if (options.publicDetail && controller.getPublicBySlug)
        router.get("/public/:slug", validate(slugParamSchema), controller.getPublicBySlug);
